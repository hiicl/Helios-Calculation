...
- **热力评分 (HeatScore)**: 统一评分机制，计算公式：  
  ...
  **计算示例**  
  ...
  计算过程：
  1. 热度部分：\(\alpha \cdot \frac{N_i}{T} = 0.7 \cdot \frac{20}{300} = 0.7 \cdot 0.0667 \approx 0.0467\)
  2. 温度部分：\(\beta \cdot e^{-\frac{(t_{\text{now}} - t_{\text{last}})}{\tau}} = 0.3 \cdot e^{-\frac{100}{120}} = 0.3 \cdot e^{-0.833} \approx 0.3 \cdot 0.434 \approx 0.130\)
  3. 热力评分：\(\text{HeatScore} \approx 0.0467 + 0.130 = 0.1767\)
