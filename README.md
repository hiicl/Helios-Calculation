...
- **迁移执行**:
  ...
  - 后续访问直接使用本地内存副本
> **优化迁移说明**  
> - **降级→L2**: 当分片被其他服务器频繁访问时，系统将其预迁移到访问节点的主机内存（L2）。这种预迁移避免了频繁的跨网络跳板操作或占用IB/RoCE网络。
> - **降级→L3**: 当分片同时被多个跨服务器节点频繁访问时（通常≥3节点），系统将绕过常规冷却流程，直接将其迁移至L3全局缓存。这种网络优化迁移可减少单节点的IB/RoCE网络压力。
> 
> **降级机制特点**:
...
### 3.4 分层网络交互规范
Helios Calculation 严格执行分层网络隔离策略，最大化利用硬件特性：
...
graph TB
  ...
  subgraph 跨节点交互
    L2 <-->|读: RDMA/主干网| L2_Other[其他节点L2]
    L2 <-->|写: ZMQ/迁移网| L2_Other
    L2 <-->|读写: 主干网| L3[Controller内存池]
  end
```
...
- **L2↔L2 跨节点交互**:
  - 读路径：RoCE主干网 + RDMA协议（低延迟）
  - 写路径：QSFP迁移网 + ZMQ协议（高带宽）
  - 典型场景：降级迁移到其他节点L2
...
### 3.5 分片状态迁移流程
Controller 监控分片状态并执行生命周期管理，严格区分降级与冷却路径：
...
- **降级→L2**: 当分片被其他服务器频繁访问时，通过QSFP迁移网预迁移到目标节点主机内存
- **降级→L3**: 当分片被≥3节点频繁访问时，通过QSFP迁移网直接降级至L3
- **冷却迁移**: 通过RoCE数据主干网迁移到L3
